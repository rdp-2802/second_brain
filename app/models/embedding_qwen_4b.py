...
#will have to upgrade the plan. nevertheless we will someday
...
documents = pd.DataFrame(Documents, columns = ["content"])
queries = pd.DataFrame(Queries, columns = ["content"])

documents["embedding"] = documents["content"].apply(lambda x: client.feature_extraction(x,model = "Qwen/Qwen3-Embedding-4B"))
queries["embedding"] = queries["content"].apply(lambda x: client.feature_extraction(x,model = "Qwen/Qwen3-Embedding-4B"))

doc_matrix = np.vstack(documents["embedding"])
query_matrix = np.vstack(queries["embedding"])

# Compute cosine similarity matrix
similarity_score = cosine_similarity(doc_matrix, query_matrix)
